...
import { HeroFocalEditor } from './HeroFocalEditor';
...
export function HeroVisualSection({ weddingData }: { weddingData: Record<string, unknown> | null }) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      // Server-side smart framing: the upload route returns the couple's joint
      // centroid (focal) for hero images — persist it together with the URL so
      // mobile crops anchor on both partners. Null (detection unavailable)
      // keeps the legacy centre crop.
      const focal = (uploadResult as { focal?: { x: number; y: number } | null }).focal ?? null;
      ...
      const putBody: Record<string, unknown> = { [fieldKey]: fileUrl, [clearKey]: '' };
      if (!isVideo && focal) {
        putBody.heroFocalX = focal.x;
        putBody.heroFocalY = focal.y;
      }
      ...
      const note = uploadResult.optimised && uploadResult.originalSize
        ...
        : '';
      // Smart-framing confirmation for hero images
      const focalNote = !isVideo && focal
        ? ' · smart framing applied — preview it below'
        : '';
      toast({ title: 'Success', description: `${isVideo ? 'Video' : 'Image'} updated${note}${focalNote}` });
    } catch (err) {
    ...
    <>
      <Card className="border-charcoal-ink/5 shadow-none">
        <CardContent className="p-4 space-y-3">
          ...
          {/* Mobile framing preview & controls — only meaningful for images */}
          {heroImgUrl && !heroVideoUrl && (
            <HeroFocalEditor heroImageUrl={heroImgUrl} weddingData={weddingData} />
          )}
