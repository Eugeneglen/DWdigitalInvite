...
import { detectHeroFocal } from '@/lib/hero-focal';
import {
  uploadFile,
  isMimeTypeAllowed,
  isImageMime,
 ...
 *
 * Response (201):
 *   { url, fileName, mimeType, fileSize, category, originalSize?, optimised?, focal? }
 *   — focal: { x, y, confidence } | null — hero images only; the joint centroid
 *     of all detected people for mobile crop anchoring. null = keep centre crop.
 *
...
export async function POST(req: NextRequest) {
  try {
    ...
    //    (Explicit `Buffer` annotation: Node 22 typings make `Buffer.from()`
    //    produce Buffer<ArrayBuffer> while sharp returns Buffer<ArrayBufferLike>
    //    — the widened annotation keeps both assignments type-valid.)
    ...
    if (isImageMime(file.type)) {
      ...
      if (config) {
        try {
          ...
          finalBuffer = processed as Buffer;
    ...
    // 10. Hero images: best-effort focal-point detection (AFTER the file is
    //     stored, so detection can never fail the upload). Returns the couple's
    //     joint centroid for mobile crop anchoring — the client persists it via
    //     PUT /api/cms/wedding. Any failure returns null → legacy centre crop.
    //     Videos and non-hero categories are skipped entirely.
    let focal: { x: number; y: number; confidence: string } | null = null;
    if (category === 'hero' && isImageMime(finalMimeType)) {
      focal = await detectHeroFocal(finalBuffer, finalMimeType);
    }

    // 11. Return the public URL + metadata
    return NextResponse.json(
      {
        ...result,
